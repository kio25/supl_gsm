...
const
     nsg=5; kol_p=4;//кол-во полей для генератора
type
  TFr1 = class(TForm)
    Label4: TLabel;
    Label5: TLabel;
...
begin
 ...
 mas_sg[1,1]:='Підрозділ'; mas_sg[1,2]:='t.DEPT#'; mas_sg[1,3]:='999999'; mas_sg[1,4]:='за підрозділом ';
  ...
  mas_sg[3,1]:='Інв.номер постач.'; mas_sg[3,2]:='nvl(f.fund#,0)'; mas_sg[3,3]:=' '; mas_sg[3,4]:='за інв.номером постачальника ';
  mas_sg[3,5]:='fund';


  mas_sg[4,1]:='Інв.номер споживач.'; mas_sg[4,2]:='nvl(fa.fund#,0)'; mas_sg[4,3]:=' '; mas_sg[4,4]:=' ';
  mas_sg[4,5]:='find_avto';



  mas_sg[5,1]:='Дата операції'; mas_sg[5,2]:='t.oprdate'; mas_sg[5,3]:=' '; mas_sg[5,4]:=' ';
  mas_sg[5,5]:='oprdate';
 ...
 for i:=1 to nsg -1 do
  ...
  if length(trim(Fr1.sg.cells[6,i]))<>0                             // итоги
    then  begin mas_order[strtoint(fr1.sg.cells[1,i]),3]:='1' ;
                mas_order[strtoint(fr1.sg.cells[1,i]),2]:='Всього за '+mas_sg[i,4];
           end
  ...
  mas_order[strtoint(fr1.sg.cells[1,5]),1]:=mas_sg[5,2]; //сортировка
  mas_order[strtoint(fr1.sg.cells[1,5]),4]:=mas_sg[5,5]; //имена полей
     end;


  if length(trim(Fr1.sg.cells[6,5]))<>0                             // итоги
    then  begin mas_order[strtoint(fr1.sg.cells[1,5]),3]:='1' ;
                mas_order[strtoint(fr1.sg.cells[1,5]),2]:='Всього за '+mas_sg[i,5];
           end
    ...
    if dmall.ODSr1.RecordCount=0
      then begin
       ...
       Sheet.select;
        ...
        Sheet.Cells[1,2]:='Витрати ПММ';
        Sheet.Cells[2,2]:='за період з '+datetostr(fr1.DateTimePicker1.Date)+' по '+datetostr(fr1.DateTimePicker2.Date);
 ...
 begin
    ...
    Sheet.Cells[index,10]:=dmall.ODSr1FNDNAME.Value;
     if DMall.ODSr1CARD_AVTO.Value=0
       then  Sheet.Cells[index,11]:='Списання на виробництво'
...
{    if DMall.ODSr1CARD_AVTO.Value=0
       then  Sheet.Cells[index,29]:='Списання на виробництво'
       ...
       begin
          ...
          then begin
            ...
            if dmall.ODS_.RecordCount<>0
               then  nam_itog:=' інв.номером споживача '+dmall.ODS_.Fields[0].value;
            dmall.ODS_.Close;

                   end

            else  nam_itog:=' cписання на виробництво'
