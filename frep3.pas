...
begin
 ...
 for i:=1 to nsg  do
   ...
   mas_sg[1,1]:='Підрозділ'; mas_sg[1,2]:='m.DEPT#'; mas_sg[1,3]:='999999'; mas_sg[1,4]:='за підрозділом ';
  mas_sg[1,5]:='dept#';

  mas_sg[2,1]:='Номенклатура'; mas_sg[2,2]:='m.PROD#'; mas_sg[2,3]:='99999999'; mas_sg[2,4]:='за номенклатурою ';
  mas_sg[2,5]:='prod#';
 ...
 for i:=1 to nsg  do
  ...
  if length(trim(Fr3.sg.cells[6,i]))<>0                             // итоги
    then  begin mas_order[strtoint(fr3.sg.cells[1,i]),3]:='1' ;
                mas_order[strtoint(fr3.sg.cells[1,i]),2]:='Всього за '+mas_sg[i,4];
           end
      ...
      XLApp:=CreateOleObject('Excel.Application');
       ...
       Sheet.select;
        ...
        // значения по которым накапливаем суммы
         ...
         Sheet.Cells[1,1]:='Оборотна відомість по ПММ';
        Sheet.Cells[2,1]:='за період з '+datetostr(fr3.DateTimePicker1.Date)+' по '+datetostr(fr3.DateTimePicker2.Date);
